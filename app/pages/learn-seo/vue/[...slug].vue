...
const [{ data: page }, { data: surround }] = await Promise.all([
  useAsyncData(`docs-${route.path}`, () => queryCollection('learnSeoVue').path(route.path).first()),
  useAsyncData(`docs-${route.path}-surround`, () => queryCollectionItemSurroundings('learnSeoVue', route.path, {
...
const repoLinks = computed(() => [
  {
    ...
    to: `https://github.com/harlan-zw/nuxtseo.com/edit/main/content/learn-seo/vue/${page.value.id.split('/').slice(2).join('/')}`,
    target: '_blank',
  },
])
</script>
